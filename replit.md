...
**Fixed Critical Token Addition Bug in Testing System (August 12, 2025)**: Resolved critical bug where simulated payments were replacing tokens instead of adding to existing balance. Testing system now properly uses additive token purchases - new tokens are added to current balance, and totalTokensPurchased tracks cumulative purchases correctly. Token display format now works as intended: "current tokens / total tokens purchased" where total updates with each purchase. Both payment simulation and referral simulation now use addTokensPurchase() method for proper additive behavior.
