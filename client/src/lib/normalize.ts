...
// Helper to format weaknesses data from comparison analysis
function formatWeaknessesData(weaknessesData: any, athleteNames: string[]): string {
  if (!weaknessesData) return 'Weaknesses analysis not available';
  
  let formatted = '';
  
  const name1 = athleteNames[0] || 'Athlete 1';
  const name2 = athleteNames[1] || 'Athlete 2';
  
  if (weaknessesData.athlete1 && Array.isArray(weaknessesData.athlete1)) {
    formatted += `**${name1} Weaknesses:**\n\n`;
    
    weaknessesData.athlete1.forEach((weakness: any, index: number) => {
      formatted += `${index + 1}. **${weakness.title || 'Weakness'}**\n`;
      if (weakness.description) {
        formatted += `   ${weakness.description}\n`;
      }
      if (weakness.impact) {
        formatted += `   *Impact:* ${weakness.impact}\n`;
      }
      if (weakness.exploitation) {
        formatted += `   *Exploitation:* ${weakness.exploitation}\n`;
      }
      formatted += '\n';
    });
  }
  
  if (weaknessesData.athlete2 && Array.isArray(weaknessesData.athlete2)) {
    formatted += `**${name2} Weaknesses:**\n\n`;
    
    weaknessesData.athlete2.forEach((weakness: any, index: number) => {
      formatted += `${index + 1}. **${weakness.title || 'Weakness'}**\n`;
      if (weakness.description) {
        formatted += `   ${weakness.description}\n`;
      }
...
// Helper to format head-to-head data from comparison analysis
function formatHeadToHeadData(headToHeadData: any, athleteNames: string[]): string {
  if (!headToHeadData) return 'Head-to-head analysis not available';
  
  let formatted = `**Head-to-Head Analysis**\n\n`;
  
  const name1 = athleteNames[0] || 'Athlete 1';
  const name2 = athleteNames[1] || 'Athlete 2';
  
  // Prediction
  if (headToHeadData.prediction) {
    const predicted = headToHeadData.prediction === 'athlete1' ? name1 : name2;
    formatted += `**Predicted Winner:** ${predicted}\n`;
    if (headToHeadData.confidence) {
      formatted += `**Confidence:** ${headToHeadData.confidence}%\n\n`;
    }
  }
  
  // Reasoning
  if (headToHeadData.reasoning) {
    formatted += `**Analysis:**\n${headToHeadData.reasoning}\n\n`;
  }
  
  // Key Factors
  if (headToHeadData.keyFactors && Array.isArray(headToHeadData.keyFactors)) {
    formatted += `**Key Factors:**\n`;
    headToHeadData.keyFactors.forEach((factor: string, index: number) => {
  ...
  // Scenario
  if (headToHeadData.scenario) {
    formatted += `**Match Scenario:**\n${headToHeadData.scenario}\n\n`;
  }
  
  // Tactical Advice
  if (headToHeadData.tacticalAdvice) {
    formatted += `**Tactical Advice:**\n`;
    if (headToHeadData.tacticalAdvice.forAthlete1) {
      formatted += `\n**For ${name1}:**\n${headToHeadData.tacticalAdvice.forAthlete1}\n`;
    }
    if (headToHeadData.tacticalAdvice.forAthlete2) {
      formatted += `\n**For ${name2}:**\n${headToHeadData.tacticalAdvice.forAthlete2}\n`;
    }
  }
...
// Helper to format competition history data from comparison analysis
function formatCompetitionHistoryData(competitionHistoryData: any, athleteNames: string[]): string {
  if (!competitionHistoryData) return 'Competition history not available';
  
  let formatted = `**Competition History Analysis**\n\n`;
  
  const name1 = athleteNames[0] || 'Athlete 1';
  const name2 = athleteNames[1] || 'Athlete 2';
  
  // Analysis narrative
  if (competitionHistoryData.analysis) {
    formatted += `${competitionHistoryData.analysis}\n\n`;
  }
  
  // Individual athlete histories
  if (competitionHistoryData.athlete1History) {
    formatted += `**${name1} Competition History:**\n`;
    if (Array.isArray(competitionHistoryData.athlete1History)) {
      competitionHistoryData.athlete1History.forEach((comp: any, index: number) => {
        formatted += `${index + 1}. ${typeof comp === 'string' ? comp : comp.event || comp.name || 'Competition'}\n`;
        if (comp.result) formatted += `   Result: ${comp.result}\n`;
        if (comp.date) formatted += `   Date: ${comp.date}\n`;
      });
    } else if (typeof competitionHistoryData.athlete1History === 'string') {
      formatted += competitionHistoryData.athlete1History;
    }
    formatted += '\n';
  }
  
  if (competitionHistoryData.athlete2History) {
    formatted += `**${name2} Competition History:**\n`;
    if (Array.isArray(competitionHistoryData.athlete2History)) {
      competitionHistoryData.athlete2History.forEach((comp: any, index: number) => {
        formatted += `${index + 1}. ${typeof comp === 'string' ? comp : comp.event || comp.name || 'Competition'}\n`;
        if (comp.result) formatted += `   Result: ${comp.result}\n`;
        if (comp.date) formatted += `   Date: ${comp.date}\n`;
      });
    } else if (typeof competitionHistoryData.athlete2History === 'string') {
      formatted += competitionHistoryData.athlete2History;
    }
    formatted += '\n';
  }
  
  // Direct encounters
  if (competitionHistoryData.directEncounters) {
    formatted += `**Direct Encounters:**\n`;
    if (Array.isArray(competitionHistoryData.directEncounters)) {
      competitionHistoryData.directEncounters.forEach((encounter: any, index: number) => {
        formatted += `${index + 1}. ${typeof encounter === 'string' ? encounter : encounter.event || 'Match'}\n`;
        if (encounter.winner) formatted += `   Winner: ${encounter.winner}\n`;
        if (encounter.score) formatted += `   Score: ${encounter.score}\n`;
      });
    } else if (typeof competitionHistoryData.directEncounters === 'string') {
      formatted += competitionHistoryData.directEncounters;
    }
  }
...
export function normalizeDevelopmentPlan(raw: any): GoalBasedPlan | null {
  try {
    ...
    // If data already has goalAnalysis at root, use it directly
    let content = data.goalAnalysis ? data : extractContent(data);
    
    // Handle string-encoded content after extraction
    ...
    if (content.title) {
      ...
      if (typeof data.title === 'string') {
        title = { en: data.title };
      } else if (data.title.en) {
        title = data.title;
      }
    }
    
    // Extract goal analysis - try both root level and content level
    let goalAnalysis: GoalArea[] = [];
    
    // Try different possible structures (check root data first, then content)
    const goals = data.goalAnalysis || content.goalAnalysis || content.goals || content.goal_analysis || 
    ...
    // Calculate counts - use root level counts if available
    ...
    const counts = data.counts && typeof data.counts === 'object' ? {
      goals: data.counts.goals || goalAnalysis.length,
      exercises: data.counts.exercises || totalExercises,
      videos: data.counts.videos || totalVideos
    } : {
    ...
    return {
      version: data.version || content.version || "1.0",
      id: data.id || content.id || generateId(),
      title,
      overview: data.overview || content.overview || content.summary,
      goalAnalysis,
      counts,
      intro: data.intro || content.intro || {
        overview: data.overview || content.overview || "Personalized development plan",
        structure: `${counts.goals} focus areas with ${counts.exercises} exercises`
      }
    };
  } catch (error) {
...
export function normalizeNutritionPlan(raw: any): StructuredNutritionPlan | null {
  try {
    ...
    // Handle double-encoded JSON (when plan field contains a stringified JSON)
    if (typeof content === 'string') {
      content = safeJsonParse(content);
    }
...
export function normalizeComparison(raw: any): ComparisonViewModel | null {
  try {
    ...
    let weaknessesData: any = null;
    let headToHeadData: any = null;
    let competitionHistoryData: any = null;
    let athleteNames: string[] = [];
    let athlete1Data: any = null;
    let athlete2Data: any = null;
    
    // Extract athlete data from root level (from API response)
    if (content.athlete1) {
      athlete1Data = content.athlete1;
    }
    if (content.athlete2) {
      athlete2Data = content.athlete2;
    }
      ...
      for (const key of tabKeys) {
        ...
        switch (key) {
          ...
          case 'strengths':
            ...
            break;
          case 'weaknesses':
            if (parsedResponse.weaknesses) {
              weaknessesData = parsedResponse.weaknesses;
            }
            break;
          case 'headToHead':
            if (parsedResponse.headToHead) {
              headToHeadData = parsedResponse.headToHead;
            }
            break;
          case 'competitionHistory':
            if (parsedResponse.competitionHistory) {
              competitionHistoryData = parsedResponse.competitionHistory;
            }
            break;
        }
      }
    ...
    // Weaknesses section - use extracted data first, then fallback to detailed analysis
    if (weaknessesData) {
      tabs.weaknesses = formatWeaknessesData(weaknessesData, athleteNames);
    ...
    // Head-to-Head section - use extracted data first
    if (headToHeadData) {
      tabs.headToHead = formatHeadToHeadData(headToHeadData, athleteNames);
    } else if (detailedAnalysis) {
      tabs.headToHead = formatHeadToHead(detailedAnalysis.athlete1, detailedAnalysis.athlete2);
    }
    
    // Competition History section - use extracted data
    if (competitionHistoryData) {
      tabs.competitionHistory = formatCompetitionHistoryData(competitionHistoryData, athleteNames);
    }
