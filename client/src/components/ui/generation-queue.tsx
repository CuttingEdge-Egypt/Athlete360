  ...
  return (
    <div className={`fixed bottom-2 sm:bottom-4 z-50 w-[calc(100vw-1rem)] sm:w-auto sm:max-w-md ${isArabic ? 'left-2 sm:left-4' : 'right-2 sm:right-4'}`} dir={isArabic ? 'rtl' : 'ltr'}>
      <Card className="bg-athlete-gray-800 border-gray-600 shadow-2xl">
        <div className="flex items-center justify-between p-1.5 sm:p-3 border-b border-gray-600">
          ...
          <div className="flex items-center gap-0.5 sm:gap-1">
            <Button
              ...
              className="h-6 w-6 sm:h-6 sm:w-6 p-0 text-gray-400 hover:text-gray-200 text-xs sm:text-sm"
            >
              {isMinimized ? '▲' : '▼'}
            </Button>
            <Button
          ...
          <CardContent className="p-0 max-h-32 sm:max-h-64 overflow-y-auto">
            <div className="space-y-0.5 sm:space-y-1">
              {queue.map((item) => (
                <div
                  key={item.id}
                  className="flex items-start sm:items-center justify-between p-1.5 sm:p-3 hover:bg-athlete-gray-700 transition-colors border-b border-gray-700 last:border-b-0"
                >
                  <div className="flex-1 min-w-0 pr-1.5 sm:pr-2">
                    <div className="flex items-center gap-1 sm:gap-2 mb-0.5 sm:mb-1 flex-wrap">
                      ...
                      <span className="text-[10px] sm:text-sm font-medium text-gray-200 truncate max-w-[100px] sm:max-w-none">
                        {item.athleteName}
                      </span>
                      <Badge 
                        variant="outline" 
                        className="text-[9px] sm:text-xs border-gray-500 text-gray-300 text-center whitespace-nowrap py-0 px-1 sm:px-2"
                      >
