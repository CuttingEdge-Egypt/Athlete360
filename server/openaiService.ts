...
export async function generateAthleteImage(name: string, sport: string, nationality?: string, personalInfo?: PersonalInfo): Promise<string | null> {
  try {
    ...
    const physicalDesc = personalInfo?.height || personalInfo?.category ? 
      ` Physical characteristics: ${personalInfo.height || 'athletic height'}, ${personalInfo.category ? `${personalInfo.category} category` : 'athletic build'}` : '';
...
export async function getAthletePersonalInfo(name: string, sport: string, nationality?: string): Promise<PersonalInfo> {
  ...
  try {
    ...
    return {
      ...
      category: result.category === "N/A" ? undefined : result.category,
...
// Competitive History generation using Gemini-2.5-pro
export async function generateCompetitiveHistory(athleteName: string, sport: string, nationality?: string): Promise<any> {
  const currentDate = new Date().toISOString().split('T')[0];
  
  // Import genAI from geminiService
  const { genAI, cleanJsonResponse } = await import('./geminiService');
  
  // Simple competitive history prompt
  const prompt = `Search the web for authentic competitive history for athlete "${athleteName}" from ${nationality || 'unknown nationality'} in ${sport}.

Your return must include the competitions the player participated in through their career.
...
export async function generateSpecificAnalysis(athleteName: string, sport: string, analysisType: string, athleteData?: any, customPrompt?: string, language: string = 'en'): Promise<any> {
  ...
  if (analysisType === 'rank' && athleteData) {
    // Use the competitive history generation
    return await generateCompetitiveHistory(athleteName, sport, athleteData.country);
...
- Only provide data if you find authentic, verifiable information through web search`;
  ...
  try {
    // Use o3 for beat-strategies (advanced reasoning), gpt-5 for strengths/weaknesses
    const model = analysisType === 'beat-strategies' ? 'o3' : 'gpt-5';
    console.log(`📊 [${model.toUpperCase()}] Generating ${analysisType} for ${athleteName}...`);
...
export async function searchAthleteImage(athleteName: string, sport?: string, nationality?: string, personalInfo?: any): Promise<string | null> {
  try {
    ...
    // Use the proper 4-step image search that worked for CJ and Vito
...
async function searchAthleteImageWithAI(athleteName: string, sport?: string, nationality?: string, personalInfo?: any): Promise<string | null> {
  try {
    ...
    if (personalInfo) {
      ...
      if (personalInfo.category && !personalInfo.category.includes('N/A')) {
        // Add category to search terms (e.g., "M-54", "Sabre")
        searchTerms.push(personalInfo.category);
      }
    }
...
export async function translateNameToArabic(englishName: string, sport: string, country: string): Promise<string | null> {
  try {
    console.log(`🌐 Translating "${englishName}" (${sport}, ${country}) to Arabic...`);
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a multilingual sports expert. Translate athlete names from English to Arabic. Return ONLY the Arabic name, nothing else. If the name is already in Arabic or cannot be translated, return the original name."
        },
        {
          role: "user",
          content: `Translate this athlete's name to Arabic: "${englishName}"
Sport: ${sport}
Country: ${country}

Return ONLY the Arabic name with proper diacritics if applicable.`
        }
      ],
      temperature: 0.3,
      max_tokens: 100
    ...
    const arabicName = response.choices[0]?.message?.content?.trim();
    
    if (!arabicName || arabicName === englishName) {
      console.log(`⚠️ No Arabic translation available for "${englishName}"`);
      return null;
    }

    console.log(`✅ Translated "${englishName}" to "${arabicName}"`);
    return arabicName;

  } catch (error) {
    console.error(`❌ Error translating name to Arabic:`, error);
    return null;
  }
}

export async function translateNameToEnglish(arabicName: string, sport?: string): Promise<string> {
  try {
    console.log(`🌐 Translating Arabic name "${arabicName}" to English using GPT-4o...`);
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a multilingual sports name translator. Your ONE JOB is to convert names from Arabic script to English/Latin script. You MUST ALWAYS return ONLY Latin alphabet characters (A-Z, a-z). ABSOLUTELY NO Arabic unicode characters are allowed in your response."
        },
        {
          role: "user",
          content: `Convert this name to English/Latin script: ${arabicName}
${sport ? `Sport: ${sport}` : ''}

RULES:
1. Return ONLY Latin alphabet characters (A-Z)
2. NO Arabic/Unicode characters allowed
3. If you know the athlete, use their official English name
4. If unknown, provide phonetic transliteration
5. Example: علي أبو العينين → Ali Abou Eleinen
...
OUTPUT ONLY THE NAME, NO EXPLANATIONS:`
    ...
    let englishName = response.choices[0]?.message?.content?.trim() || '';
    
    // Check if the response still contains non-Latin characters
    const arabicRegex = /[\u0600-\u06FF]/;
    const hasOnlyLatin = /^[a-zA-Z\s\-'.]+$/.test(englishName);
    
    if (!englishName || arabicRegex.test(englishName) || !hasOnlyLatin) {
      console.log(`⚠️ GPT translation failed to return Latin-only text. Response: "${englishName}"`);
      console.log(`📝 Using Arabic name - Gemini web search will find English version during personal info fetch`);
      return arabicName;
    }

    console.log(`✅ Successfully translated "${arabicName}" → "${englishName}"`);
    return englishName;

  } catch (error) {
    console.error(`❌ Error translating name to English:`, error);
    console.log(`📝 Using Arabic name - Gemini web search will find English version`);
    return arabicName;
  }
}