...
Return only real, working image URLs in JSON format.`;
    ...
    try {
      ...
      if (Array.isArray(result)) {
        console.log(`🎯 Gemini provided ${result.length} URLs for ${athlete.name} (array format)`);
    ...
    console.log(`❌ Could not parse image URLs from Gemini response for ${athlete.name}`);
    return [];

  } catch (error) {
    console.error('Error getting direct image URLs from Gemini:', error);
    return [];
  }
}
