  ...
  app.post('/api/athletes/search-suggestions', isAuthenticated, async (req: any, res) => {
    try {
      ...
      res.json({
        ...
        requiresSelection: searchResults.requiresSelection,
      ...
      if (error instanceof Error && error.message.includes('AI_SEARCH_FAILED')) {
        return res.status(404).json({
          message: "No athletes found matching your search. Please try a different name or check spelling.",
          error: "no_athletes_found"
        });
      }
      
      res.status(500).json({ 
        message: "Failed to search for athletes. Please try again.",
        error: error instanceof Error ? error.message : String(error)
      });
    }
  });

  // Create athlete from selected suggestion (Step 2)
  app.post('/api/athletes/create-from-suggestion', isAuthenticated, async (req: any, res) => {
    try {
      const { suggestion, sportId } = req.body;
      const userId = req.user.claims.sub;
      
      if (!suggestion || !sportId) {
        return res.status(400).json({ message: "Suggestion data and sport ID are required" });
      }

      console.log(`🤖 Step 2: Creating athlete from suggestion - ${suggestion.fullName}`);
      
      // Import the athlete creation function  
      const { createAthleteFromSuggestion } = await import('./openaiService.js');
      
      // Create detailed athlete profile from suggestion
      const athleteProfile = await createAthleteFromSuggestion(suggestion);
      
      // Check if athlete already exists by searching for athletes with similar name
      const existingAthletes = await storage.searchAthletesByName(athleteProfile.name, sportId);
      const existingAthlete = existingAthletes.find(a => a.name.toLowerCase() === athleteProfile.name.toLowerCase());
      ...
      const athleteData = {
        ...
        age: athleteProfile.age || suggestion.age,
        country: athleteProfile.country || suggestion.country,
        sportId: sportId,
        achievements: athleteProfile.achievements || [],
        rank: typeof athleteProfile.rank === 'number' ? athleteProfile.rank : null,
      ...
      res.json({
        ...
        rank: newAthlete.rank,
        profileImageUrl: newAthlete.profileImageUrl,
        achievements: newAthlete.achievements,
        recentNews: athleteProfile.recentNews || []
      });
      ...
      if (error instanceof Error && error.message.includes('AI_WEB_SEARCH_FAILED')) {
        return res.status(404).json({
          message: "Could not find sufficient data to create athlete profile. Please try a different athlete.",
          error: "insufficient_data"
        });
      }
      
      res.status(500).json({ 
        message: "Failed to create athlete profile. Please try again.",
        error: error instanceof Error ? error.message : String(error)
      });
    }
  });
